...
namespace udt
{
    ...
    protected:
         ...
         *
         * @param input Optional external 3D vector (uses internal vec_3D if empty)
         * @post Result stored in member 'flatten' (2D vector)
         * @warning Will skip operation if input is empty
         */
        vec<vec<T>> flatten_2dvec(const vec<vec<vec<T>>> &input = {}) const
        {
            const auto &target = input.empty() ? vec_3D : input;
            vec<vec<T>> flatten;
            if (target.empty())
         ...
         */
        vec<vec<vec<T>>> transpose(const vec<vec<vec<T>>> &input = {})
        {
            const auto &target = input.empty() ? vec_3D : input;
            vec<vec<vec<T>>> transpose_3D;
         ...
         */
        vec<long double> determinant_vec(const vec<vec<vec<T>>> &input = {}) const
        {
            const auto &target = input.empty() ? vec_3D : input;
            if (target.empty())
                throw std::runtime_error(
                    "EmptyMatrixError: Cannot calculate determinant of empty 3D vector");
            vec<long double> determinant;
            vector_2d<T> vt;
            for (const vec<vec<T>> &loopvec : target)
    ...
    public:
         ...
         */
        bool is_sizeNXMXW(int N, int M, int W, const vec<vec<vec<T>>> &v = {}) const
        {
         ...
         */
        bool is_cudeNXNXN(int N, const vec<vec<vec<T>>> &v = {})
        {
         ...
         */
        bool is_cude3X3X3(const vec<vec<vec<T>>> &v = {})
        {
         ...
         */
        bool is_cude2X2X2(const vec<vec<vec<T>>> &v = {})
        {
         ...
         */
        bool is_equal_size(const vec<vec<vec<T>>> &v)
        {
            if (v.empty() || vec_3D.empty())
         ...
         *
         * @param input3d 3D vector to copy into the new object
         */
        vector_3d(const vec<vec<vec<T>>> &input3d)
        {
            vec_3D = input3d;
        }
         ...
         *
         * @param input3d The new 3D vector to store as the main data
         * @note Overwrites any existing 3D vector content
         */
        void set(const vec<vec<vec<T>>> &input3d) noexcept
        {
            vec_3D = input3d;
        }
         ...
         */
        const vec<vec<vec<T>>> &get() const noexcept { return vec_3D; }
         ...
         */
        vec<vec<vec<T>>> get_copy() const noexcept { return vec_3D; }
         ...
         */
        const vec<vec<T>> &get(size_t index) const
        {
         ...
         * @throws out_of_range if index is invalid
         */
        vec<vec<T>> get_copy(size_t index) const
        {
            if (index >= vec_3D.size())
                throw std::out_of_range("Depth index out of range");
            return vec_3D[index];
         ...
         */
        const vec<T> &get(size_t ind1, size_t ind2) const
        {
            if (ind1 >= vec_3D.size())
                throw std::out_of_range("Depth index out of range");
         ...
         * @throws out_of_range if either index is invalid
         */
        vec<T> get_copy(size_t ind1, size_t ind2) const
        {
            if (ind1 >= vec_3D.size())
                throw std::out_of_range("Depth index out of range");
            if (ind2 >= vec_3D[ind1].size())
         ...
         */
        void at(size_t ind1, size_t ind2, vec<T> &value)
        {
            if (ind1 >= vec_3D.size())
                throw std::out_of_range("Depth index out of range");
            if (ind2 >= vec_3D[ind1].size())
         ...
         */
        void at(size_t ind1, vec<vec<T>> &value)
        {
            if (ind1 >= vec_3D.size())
                throw std::out_of_range("Depth index out of range");
            vec_3D[ind1] = value;
         ...
         * @param ind1 The depth index (first dimension).
         * @param ind2 The row index (second dimension).
         * @return vec<T> The 1D vector at the specified indices.
         * @throws std::out_of_range If either ind1 or ind2 is out of bounds.
         */
        vec<T> at(size_t ind1, size_t ind2) const
        {
            if (ind1 >= vec_3D.size())
                throw std::out_of_range("Depth index out of range");
            if (ind2 >= vec_3D[ind1].size())
         ...
         * @return vec<vec<T>> The 2D vector at the specified depth.
         * @throws std::out_of_range If the provided index is out of the 3D vector's range.
         */
        vec<vec<T>> at(size_t ind1) const
        {
            if (ind1 >= vec_3D.size())
                throw std::out_of_range("Depth index out of range");
            return vec_3D[ind1];
         ...
         *
         * @return A vec<vec<vec<T>>> containing the transposed data.
         */
        vec<vec<vec<T>>> get_transpose() { return transpose(); }
         ...
         * @return A vec<vec<T>> representing the requested 2D slice.
         * @throws std::out_of_range if index < 0 or index >= number of layers.
         */
        vec<vec<T>> get_transpose(int index) const
        {
         ...
         * @return A vec<T> representing the requested row.
         * @throws std::out_of_range if ind1 or ind2 is out of range.
         */
        vec<T> get_transpose(int ind1, int ind2) const
        {
         ...
         */
        vec<T> get_transpose(int ind1, int ind2, int ind3) const
        {
            if (ind1 < 0 || ind1 >= static_cast<int>(transpose().size()))
                throw std::out_of_range("Depth index out of range");
            if (ind2 < 0 || ind2 >= static_cast<int>(transpose()[ind1].size()))
         ...
         */
        vec<vec<T>> &get_flatten(vec<vec<T>> &input)
        {
            input = flatten_2dvec();
            return input;
        }
         ...
         */
        vec<vec<T>> get_copy_flatten() const { return flatten_2dvec(); }
         ...
         * @throws out_of_range if index is invalid
         */
        const vec<T> &get_flatten(size_t index) const
        {
         ...
         * @throws out_of_range if index is invalid
         */
        vec<T> get_copy_flatten(size_t index) const
        {
            if (index >= flatten_2dvec().size())
                throw std::out_of_range("Flatten row index out of range");
         ...
         */
        vec<long double> get_determinant_vec(const vec<vec<vec<T>>> &input = {}) const { return determinant_vec(vec); }
         ...
         */
        long double get_determinant(size_t index, const vec<vec<vec<T>>> &input = {}) const
        {
            const vec<long double> &detvec = determinant_vec(vec);
         ...
         *
         * @param input2d 2D matrix to append
         * @throws Error if fixed depth (w) is reached
         */
        void push(const vec<vec<T>> &input2d)
        {
         ...
         * @throws Error if fixed depth (w) is reached
         */
        void push(const vector_2d<T> &input2d)
        {
            if (w != -1 && w == vec_3D.size())
                throw std::runtime_error("CapacityError: Cannot exceed fixed depth");
         ...
         * @param input Row vector to append
         * @throws Error if:
         *           - Index is invalid, OR
         *           - Fixed row count (n) is reached
         *          - Row exceeds fixed column count (m)
         */
        void push(size_t index, const vec<T> &input)
        {
            if (index >= vec_3D.size())
                throw std::out_of_range("Depth index out of range");
        ...
        void operator+=(const T element)
        {
            ...
            for (vec<vec<T>> &input_2d : input_3D)
            {
                for (vec<T> &input : input_2d)
                {
                    for (T &ele : input)
                    {
                        ele += element;
         ...
         */
        void operator+=(const vec<vec<vec<T>>> &elements)
        {
         ...
         */
        vec<vec<T>> &operator[](size_t i) noexcept { return vec_3D[i]; }
         ...
         * @warning No bounds checking - use with valid indices
         */
        const vec<vec<T>> &operator[](size_t i) const noexcept { return vec_3D[i]; }

        /**
         * Assignment operator for 3D vector replacement
         *
         * @param input3d 3D vector to assign to the internal storage
         * @post Replaces current vec_3D with the input vector
         */
        void operator=(const vec<vec<vec<T>>> &input3d) noexcept { vec_3D = input3d; }

        /**
         * Equality operator for 3D vector comparison
         *
         * @param input3d 3D vector to compare against
         * @return true if:
         ...
         */
        bool operator==(const vec<vec<vec<T>>> &input3d) const noexcept
        {
            if (input3d.empty() && vec_3D.empty())
                return true;
            if (input3d.empty())
                return false;
            if (vec_3D.empty())
                return false;
            if (vec_3D.size() != input3d.size())
                return false;
            for (size_t i = 0; i < input3d.size(); i++)
            {
                if (input3d[i].size() != vec_3D[i].size())
                    return false;
                for (size_t j = 0; j < input3d[i].size(); j++)
                {
                    if (!same_v(input3d[i][j], vec_3D[i][j]))
                        return false;
                }
            }
            return true;
